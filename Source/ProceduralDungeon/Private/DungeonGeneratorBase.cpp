...
bool ADungeonGeneratorBase::AddRoomToDungeon(URoom* const& Room, const TArray<int>& DoorsToConnect, bool bFailIfNotConnected)
{
	...
	if (Room->GetRoomData()->GetSettings() != SettingsOverrides)
	{
		...
		DungeonLog_Error("Mismatching dungeon settings between dungeon %s and room %d (settings should be %s)", *GetName(), *GetNameSafe(Room->GetRoomData()), *GetNameSafe(SettingsOverrides))
		return false;
	}
